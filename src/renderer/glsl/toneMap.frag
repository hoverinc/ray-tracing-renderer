...
source: `
  ...
  uniform mediump sampler2DArray diffuseSpecularBuffer;
  ...
  vec4 getUpscaledLight(vec2 coord) {
    float meshId = getMeshId(positionBuffer, coord);

    vec2 sizef = lightScale * vec2(textureSize(positionBuffer, 0));
    ...
    for (int i = 0; i < 4; i++) {
      ...
      float isValid = getMeshId(positionBuffer, pCoord) == meshId ? 1.0 : 0.0;
  ...
  void main() {
    ...
    // #else
    //   vec4 upscaledLight = texture(diffuseSpecularBuffer, vec3(lightScale * vCoord, 0));
    // #endif
    vec4 diffuse = texture(diffuseSpecularBuffer, vec3(lightScale * vCoord, 0));
    vec4 specular = texture(diffuseSpecularBuffer, vec3(lightScale * vCoord, 1));
    vec3 albedo = texture(albedoBuffer, vCoord).rgb;
    float metalness = texture(albedoBuffer, vCoord).a;
    ...
    vec3 light = (albedo * diffuse.rgb + mix(vec3(1.0), albedo, metalness) * specular.rgb) / diffuse.a;
